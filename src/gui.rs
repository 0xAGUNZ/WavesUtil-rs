...
mod app_gui {
    ...
    use std::ops::Deref;
    use std::rc::Rc;
    pub struct WaveUtilUi {
        inner: Rc<WaveUtil>,
        default_handler: RefCell<Option<nwg::EventHandler>>,
    }
    impl nwg::NativeUi<WaveUtilUi> for WaveUtil {
        fn build_ui(mut data: WaveUtil) -> Result<WaveUtilUi, nwg::NwgError> {
            use nwg::Event as E;
                    ...
                    match evt {
                        E::OnButtonClick => {
                            ...
                            if handle == ui.disableidle_button {
                                // "Prevent" from changing checkbox when WaveUtil is running
    ...
    impl Deref for WaveUtilUi {
        type Target = WaveUtil;
        fn deref(&self) -> &WaveUtil {
            &self.inner
        }
    }
}
...
pub fn gui_init() {
    ...
    let _ui = WaveUtil::build_ui(Default::default()).expect("Failed to build UI");
    nwg::dispatch_thread_events();
}

fn restore(ui: Rc<WaveUtil>) {
